...
class regexSpider:
    ...
    #下载并写入一个url
    def downAndWriteOneUrl(self, baikeUrl, referUrl,file_object_X,bWrite):
        try:
            ...
            if(srcHtml != None and pageUrl != None):
                ...
                if(bWrite):
                    ...
                    soup = BeautifulSoup(baikeHtml, "lxml")
                    page=soup.find_all("ul","list-main-eventset")[1]
                    detail = page.find_all('li')
                    for i in detail:
                        ...
                        location = deal.find("span","loca c-gray-aset t-small").get_text().encode("utf-8")#.contents[1].contents
                        ...
                        if len(i.find("i","cell date").find_all("a")) >1:
                            invest = i.find("i","cell date").find_all("a")
                            ilist=[]
                            for j in invest:
                                investor = j.get_text()
                                ilist.append(invest)
                                ...
                                investor = i.find("i","cell date").get_text()
                    message = Url+'|'+date +'|'+company+ '|'+ area +'|'+ location +'|'+ around  +'|'+ number +'|'+ investor
                    end = message.encode("utf-8").replace('\n', '').replace('\t', '')
                    file_object_X.write(end)
                    file_object_X.write('\n')
    ...
    #抓取递归处理函数
    def crawlPages(self, url, referUrl,downUrlReg,writeUrlReg,ignoreUrlReg,file_object_X,deepMax,deepCnt):
        deepCnt += 1
        ...
        if(url not in self.doneUrls):
            ...
            downUrl,srcHtml = self.downAndWriteOneUrl(url,referUrl,file_object_X,bWrite)
                        ...
                        if subUrl != None:
                            ...
                            if downUrlReg.match(subUrl) != None:#需要处理
                                if(ignoreUrlReg.match(subUrl) == None): #且不是过滤链接
                                    print subUrl
                                    self.crawlPages(subUrl,downUrl,downUrlReg,writeUrlReg,ignoreUrlReg,file_object_X,deepMax,deepCnt)
    ...
    def spiderWorking(self, startUrlList, downUrlRegex, writeUrlRegex,ignoreUrlRegex, writeFileName,deepMax):
        ...
        file_object_X = open(writeFileName, "wb")
        file_object_X.write("Url|时间|公司|所属行业|地区|轮次|融资额|投资方")
        file_object_X.write('\n')
        ...
        try:
            for startUrl in startUrlList:
                self.crawlPages(startUrl,startUrl,downUrlReg,writeUrlReg,ignoreUrlReg,file_object_X,deepMax,0)
                sleepSec = random.randrange(3,8)
                time.sleep(sleepSec)
...
if __name__ == "__main__":
    ...
    downUrlRegex = "(https://www\.itjuzi\.com/investevents\?page=[0-9]{1,4})"
    ...
    writeUrlRegex ="(https://www\.itjuzi\.com/investevents\?page=[0-9]{1,4})|(https://www.itjuzi.com/investevents)"
