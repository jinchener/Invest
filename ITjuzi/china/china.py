...
def GetAllLink():
    ...
    for i in range(num):
        if i+1 == 1:
            url = 'http://www.chinaventure.com.cn/cvsourcemodel/financing/detail/29308.shtml'
            GetPage(url, i)
        else:
...
def GetPage(url, num):
    ...
    title= soup.find("h1","left_h1").string
    f = open("text1.txt",'wb')
    ilist=[]
    for i in fixedprice:
        data= i.find_all('td')[1].string
        ilist.append(data)
    str=title+'|'+'|'.join((ilist))
    f.write(str)
    f.close()
    print 'Ok!'
