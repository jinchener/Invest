        ...
        try:
            ...
            if(srcHtml != None and pageUrl != None):
                ...
                if(bWrite):
                    ...
                    #body=soup.find("div","content").find("div","product-show")
                    #print body
                    head = soup.find("div", "news-show")
                    title = head.find("h1").get_text().replace("\n", "").replace(" ", "").replace("\t", "").replace("\r", "")
                    page = head.find_all("p")
                    ilist = []
                    for i in page:
                        ilist.append(i.get_text().encode("utf-8").split("：")[1])
                    deal = '|'.join(ilist)
                    end = soup.find("div", "company")
                    short = end.find("img").get('alt')
                    company = end.find("div", "title").get_text().replace("\n", "").replace(" ", "").replace("\t", "").replace("\r", "")
                    #print company
                    areas = end.find("div", "title").next_siblings
                    ilist1 = []
                    for i in areas:
                        if i.find("dt") != -1:
                            ilist1.append(i.get_text().encode("utf-8").split("：")[1])
                    end1 = '|'.join(ilist1)
                    message =  '|' + title + '|' + deal + '|' + short + '|' + company + '|' + end1
                    ...
                    #print str(baikeUrl)
                    #file_object_Y.writerow([str(body)])
                    file_object_Z.write(baikeUrl+message1)
                    file_object_Z.write("\n")
...
if __name__ == "__main__":
    ...
    result.write('url|标题|投资时间|投资方|受资方|轮次|领域|金额|介绍|简称|全称|地点|领域')
    result.write("\n")
