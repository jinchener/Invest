...
class regexSpider:
    ...
    #下载并写入一个url
    def downAndWriteOneUrl(self, baikeUrl, referUrl,file_object_X,file_object_Y,bWrite):
        try:
            ...
            if(srcHtml != None and pageUrl != None):
                ...
                if(bWrite):
                    ...
                    soup1 = BeautifulSoup(baikeHtml, "lxml")
                    soup2 = soup1.find("div","main")
                    soup3 = soup2.find("div","sec")
                    #print soup3
                    ...
                    file_object_Y.writerow([str(soup3)])
                    sleepSec = random.randrange(3,8)
                    time.sleep(sleepSec)
                    #file_object_X.write(baikeUrl)

                    #file_object_Y.write(baikeHtml)
    ...
    #抓取递归处理函数
    def crawlPages(self, url, referUrl,downUrlReg,writeUrlReg,ignoreUrlReg,file_object_X,file_object_Y,deepMax,deepCnt):
        deepCnt += 1
        ...
        if(url not in self.doneUrls):
            ...
            downUrl,srcHtml = self.downAndWriteOneUrl(url,referUrl,file_object_X,file_object_Y,bWrite)
                        ...
                        if subUrl != None:
                            ...
                            if downUrlReg.match(subUrl) != None:#需要处理
                                if(ignoreUrlReg.match(subUrl) == None): #且不是过滤链接
                                    print subUrl
                                    self.crawlPages(subUrl,downUrl,downUrlReg,writeUrlReg,ignoreUrlReg,file_object_X,file_object_Y,deepMax,deepCnt)
    ...
    #		写文件url正则
    #		写文件名称
    def spiderWorking(self, startUrlList, downUrlRegex, writeUrlRegex,ignoreUrlRegex, writeFileName1,writeFileName2 ,deepMax):
        downUrlReg = re.compile(downUrlRegex)
        writeUrlReg = re.compile(writeUrlRegex)
        ignoreUrlReg = re.compile(ignoreUrlRegex)
        #file_object_X  = open(writeFileName1,'w')
        #file_object_Y  = open(writeFileName2,'w')
        ...
        #file_object_X = open(writeFileName1, "wb")
        #file_object_Y = open(writeFileName2, "wb")

        #csvfile = file('resul.csv', 'a')
        #writer = csv.writer(csvfile,delimiter='|')
        try:
            for startUrl in startUrlList:
                self.crawlPages(startUrl,startUrl,downUrlReg,writeUrlReg,ignoreUrlReg,file_object_X,file_object_Y,deepMax,0)
                sleepSec = random.randrange(3,8)
                time.sleep(sleepSec)
...
if __name__ == "__main__":
    ...
    startUrlList = ["https://www.itjuzi.com/investevents"]
    #["http://pinyin.sogou.com/dict/"]
    downUrlRegex = "(https://www\.itjuzi\.com/investevents\?page=[0-9]{1,4})|(https://www\.itjuzi\.com/investevents/[0-9]{1,5})"
    ...
    writeUrlRegex ="(https://www\.itjuzi\.com/investevents/[0-9]{1,5})"
    ...
    writeFileName1 = "data/url.csv"
    writeFileName2="data/page.csv"
    ...
    mySpider.spiderWorking(startUrlList,downUrlRegex,writeUrlRegex,ignoreUrlRegex,writeFileName1,writeFileName2,200)
    print "All Done!"
