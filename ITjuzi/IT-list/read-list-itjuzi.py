...
class regexSpider:
	...
	#下载并写入一个url		
	def downAndWriteOneUrl(self, baikeUrl, file_object_Z, bWrite):
		try:
			...
			if(srcHtml != None and pageUrl != None):
				...
				if(bWrite):
					...
					message = baikeUrl + '|' + '|'.join(ilist).replace("\n", "").replace(" ", "").replace("\t", "")
...
if __name__ == "__main__":
	...
	try:
		while 1:
			...
			mySpider.downAndWriteOneUrl(line1,file_object_Z,True)
